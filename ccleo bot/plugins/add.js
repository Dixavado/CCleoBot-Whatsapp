...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  if (!text) throw `_Digite um numero!_ \nExemplo:\n\n${usedPrefix + command + ' ' + global.owner[0]}`
  ...
  if (response[users] == 408) throw `O número saiu recentemente\nSó pode entrar por ${usedPrefix}link`
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    let teks = `Entrou @${jid.split('@')[0]} usando convite...`
    m.reply(teks, null, {
      contextInfo: {
...
handler.help = ['add/+'].map(v => v + ' 55119...,55219...')
handler.tags = ['admin']
